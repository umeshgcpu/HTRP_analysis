...
infocase <- infocase0 %>%
  pivot_longer(c(radio:othersourceinfo), names_to = "sourceofinfo", values_to = "cases") %>% 
    ...
    count(cases) %>%
    filter(cases == 1) %>%
    mutate(percentage = n/nrow(infocases)*100) %>%
...
ggplot(methods, aes(x = fct_reorder(method, percentage), y= percentage, label = round(percentage,2) , fill = method)) +
    geom_col() + 
  coord_flip() +
  geom_text(size=3, vjust = 0.5, hjust = -0.5) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
...
methodmixresp <- countusingcases %>%
 ...
 ggplot(methods, aes(x = fct_reorder(method, percentage, .desc = F), y= percentage, label = round(percentage*100,2), fill = method)) +
    geom_col() + 
   coord_flip() +
   scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
      theme_minimal() +
