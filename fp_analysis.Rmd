...
fp <- fp %>%
  mutate (lifetimeuse = ifelse (currentuse11 == 1 | everusecontra == 1 , 1, 0))
...
#Null Model
model0 <- glm(factor(fp$lifetimeuse) ~ 1, family = binomial(link = "logit"))
summary(model0)

```

#First model
model1 <- glm(factor(fp$lifetimeuse) ~ factor(fp$province) + + factor(fp$typeoflocal1) + factor(fp$respcastecode) + factor(fp$respcastecode) + factor(fp$q107) + factor(fp$agegroup) + factor(fp$respedungrp) + factor(fp$q112) + factor(fp$quintile) + factor(fp$ageatmarriagegroup) + factor(fp$q119) + factor(fp$livingchildgrp) + factor(fp$husbedungrp) + factor(fp$q113) + factor(fp$q120a) + factor(fp$spouseagediffgrp) + factor(fp$q122)  , family = binomial(link = "logit"))
summary(model1)
...
```{r results = "asis"}
#Make variables ready for final model: Remove religion and  recode other variables: age group, education,age at marriage, husband education

fp <- fp %>%
  mutate(magegroup = ifelse (agegroup == "15-19", "15-19",
                                 ifelse(agegroup == "20-24", "20-24",
                                        ifelse(agegroup == "25-29", "25-29",
                                               ifelse (agegroup == "30-34", "30-34",
                                                       ifelse(agegroup == "35-39", "35-39","40-49"))))))
fp <- fp %>%
  mutate(mrespedungrp = ifelse(respedungrp == "None", "None",
                               ifelse(respedungrp == "Primary", "Primary",
                                      ifelse(respedungrp == "Secondary", "Secondary", "+2 and above"))))


fp <- fp %>%
  mutate(mageatmarriagegroup = ifelse (ageatmarriagegroup == "14 or less", "14 or less",
                                       ifelse(ageatmarriagegroup == "15-19", "15-19",
                                              ifelse(ageatmarriagegroup == "20-24", "20-24", "25 and above"))))
  

fp <- fp %>%
  mutate(mhusbedungrp = ifelse(husbedungrp == "None", "None",
                               ifelse(husbedungrp == "Primary", "Primary",
                                      ifelse(husbedungrp == "Secondary", "Secondary", "+2 and above"))))

fp <- fp %>%
  mutate(mq122 = ifelse(q122 == "Nuclear", "Nuclear", "Joint or extended"))


fp <- fp %>%
  mutate(mrespcastecode = ifelse(respcastecode1 == "Hill Brahmin" | respcastecode1 ==  "Hill Chhettri" | respcastecode1 == "Tarai Brahmin/Chhettri", "Brahmin/Chhettri",  ifelse(respcastecode1 == "Hill Dalit", "Hill Dalit",
                                       ifelse(respcastecode1 == "Tarai Dalit", "Tarai Dalit",
                                              ifelse(respcastecode1 == "Hill Janajati", "Others",
                                                      ifelse(respcastecode1 == "Tarai Janajati", "Others",
                                                             ifelse(respcastecode1 == "Muslim", "Muslim", "Others")))))))


fp <- fp %>%
  mutate(mq113 = ifelse(q113 == "Agriculture", "Agriculture",
                        ifelse(q113 == "Government service", "Government/Private Service",
                                               ifelse(q113 == "Private job", "Government/Private Service",
                                                      ifelse(q113 == "Daily wage / labourer", "Daily wage / labourer / Others",
                                                             ifelse(q113 == "Business", "Business",
                                                                    ifelse(q113 == "Foreign employment", "Foregin employment", "Daily wage / labourer / Others")))))))

fp <- fp %>%
  mutate(mq112 = ifelse (q112 == "Housewife", "Housewife or Unemployed",
                         ifelse(q112 == "Agriculture", "Agriculture",
                                ifelse(q112 == "Paid employment (Private job, Daily wage, Foreign employment)", "Paid employment (Private job, Daily wage, Foreign employment)", "Housewife or Unemployed" ))))


  

#Relevel variables and specify reference variabe

fp$province <-  as.factor(fp$province)
fp$province <- relevel(fp$province, ref = "one")

fp$magegroup <-  as.factor(fp$magegroup)
fp$magegroup <- relevel(fp$magegroup, ref = "40-49")

fp$mrespedungrp <-  as.factor(fp$mrespedungrp)
fp$mrespedungrp <- relevel(fp$mrespedungrp, ref = "None")

fp$quintile <-  as.factor(fp$quintile)
fp$quintile <- relevel(fp$quintile, ref = "[ 7.14,27.9)")

fp$mq112 <-  as.factor(fp$mq112)
fp$mq112 <- relevel(fp$mq112, ref = "Housewife or Unemployed" )


fp$mageatmarriagegroup <-  as.factor(fp$mageatmarriagegroup)
fp$mageatmarriagegroup <- relevel(fp$mageatmarriagegroup, ref = "14 or less")

fp$mageatmarriagegroup <-  as.factor(fp$mageatmarriagegroup)
fp$mageatmarriagegroup <- relevel(fp$mageatmarriagegroup, ref = "14 or less")

fp$q119 <-  as.factor(fp$q119)
fp$q119 <- relevel(fp$q119, ref = "Yes")

fp$mhusbedungrp <-  as.factor(fp$mhusbedungrp)
fp$mhusbedungrp <- relevel(fp$mhusbedungrp, ref = "None")

fp$q120a <-  as.factor(fp$q120a)
fp$q120a <- relevel(fp$q120a, ref = "Staying elsewhere")

fp$spouseagediffgrp <- as.factor(fp$spouseagediffgrp)
fp$spouseagediffgrp <- relevel(fp$spouseagediffgrp, ref = "Woman older")

fp$mq122 <- as.factor(fp$mq122)
fp$mq122 <- relevel(fp$mq122, ref = "Joint or extended")




#Final model
model2 <- glm(factor(fp$lifetimeuse) ~ factor(fp$province) + + factor(fp$typeoflocal1) + factor(fp$mrespcastecode) +  factor(fp$magegroup) + factor(fp$mrespedungrp) + factor(fp$mq112) + factor(fp$quintile) + factor(fp$mageatmarriagegroup) + factor(fp$q119) + factor(fp$livingchildgrp) + factor(fp$mhusbedungrp) + factor(fp$mq113) + factor(fp$q120a) + factor(fp$spouseagediffgrp) + factor(fp$mq122) + factor(fp$autoassoc)  , family = binomial(link = "logit"))


summary(model2)
od1 <- exp(model2$coefficients)
od1
confint(model2)
...
# Let's see the model summary from summ function of jtools package
summ(model2, confint = TRUE,  pvals = TRUE, exp = TRUE, vifs = TRUE, model.fit = TRUE )
summ(model2, robust = F)


HL3 <- hoslem.test(x= model2$y, y = fitted(model2), g=10)
HL3
