...
# Type of local level
fp <- fp %>%
  mutate(typeoflocal = ifelse(locallvlname == "aathbis"|locallvlname == "chapakot"|locallvlname == "panchadewal"|locallvlname == "bhajani"|locallvlname == "maharajganj" | locallvlname == "mahalaxmidhan", "Municipality", 
...
#Urbanrural
  fp <- fp %>%
  mutate(typeoflocal1 = ifelse(locallvlname == "aathbis"|locallvlname == "chapakot"|locallvlname == "panchadewal"|locallvlname == "bhajani"|locallvlname == "maharajganj"|locallvlname == "lalitpur" | locallvlname == "mahalaxmidhan", "Urban", "Rural"))
...
fp$q107 <- factor(as.numeric(fp$q107), levels = c( '1', '2', '3', '4', '5', '6'), labels = c( "Hindu", "Buddhist", "Muslim", "Kirant", "Christian", "Other"))
...
ggplot(inclusioncase, aes(x = fct_reorder(group, percentage), y= percentage/100, label = round(percentage,2) , fill = group)) +
    geom_col() + 
  coord_flip() +
...
ggplot(infocase, aes(x = fct_reorder(sourceofinfo, percentage), y= percentage, label = round(percentage,2) , fill = sourceofinfo)) +
    geom_col() + 
  coord_flip() +
  geom_text(size=3, vjust = 0.5, hjust = -0.5) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
...
methodcase <- countusingcases %>%
  pivot_longer(c(usefemster:useothertrad), names_to = "method", values_to = "usestatus") %>% 
    ...
    count(usestatus) %>%
    filter(usestatus == 1) %>%
    mutate(percentage = n/nrow(countusingcases)*100) %>%
    arrange(desc(percentage)) %>%
  print()

ggplot(methodcase, aes(x = fct_reorder(method, percentage), y= percentage, label = round(percentage,2) , fill = method)) +
    geom_col() + 
  coord_flip() +
  geom_text(size=3, vjust = 0.5, hjust = -0.5) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
...
methodresp <- countusingcases %>%
 ...
 ggplot(methodresp, aes(x = fct_reorder(method, percentage, .desc = F), y= percentage, label = round(percentage,2), fill = method)) +
   ...
   scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
      theme_minimal() +
   geom_text(size=3, vjust = 0.5, hjust = -0.00000001) +
   labs(x="", 
       y= " Percentage of responses", 
       title = "Current users of family planning") +
   theme(plot.title = element_text(hjust = 0.5)) +
