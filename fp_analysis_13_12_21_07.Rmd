...
#Make variables ready for final model: Remove religion and  recode other variabales: age group, education,age at marriage, husband education,
...
#Final model
model2 <- glm(factor(fp$everusecontra) ~ factor(fp$province) + + factor(fp$typeoflocal1) + factor(fp$respcastecode) +  factor(fp$magegroup) + factor(fp$mrespedungrp) + factor(fp$q112) + factor(fp$quintile) + factor(fp$mageatmarriagegroup) + factor(fp$q119) + factor(fp$livingchildgrp) + factor(fp$mhusbedungrp) + factor(fp$q113) + factor(fp$q120a) + factor(fp$spouseagediffgrp) + factor(fp$mq122)  , family = binomial(link = "logit"))
summary(model2)
od <- exp(model2$coefficients)
od
confint(model2)

# Let's see the model summary from summ function of jtools package
summ(model2, confint = TRUE,  pvals = TRUE, exp = TRUE)
summ(model2, robust = F)


VIF(model2)

HL3 <- hoslem.test(x= model1$y, y= fitted(model1), g=10)
HL3
